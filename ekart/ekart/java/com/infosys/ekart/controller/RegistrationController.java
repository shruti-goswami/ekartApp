...
public class RegistrationController {
	...
	public ModelAndView addCustomer(@Valid @ModelAttribute("command") User user, BindingResult result, ModelMap model) {
		...
		if(result.hasErrors()) {
			...
			try {
				logger.info("the command is try" + command);
			registrationService.registerUser(user);
				modelAndView = new ModelAndView(register,command,user);
				modelAndView.addObject("successMessage","You are successfully registered with Ekart. Use your user id and password to login!");
				
			} catch(EmailIdAlreadyInUseException ex) {
				if(ex.getMessage().contains("RegistrationService")) {
					modelAndView = new ModelAndView(register);
					modelAndView.addObject(command,user);
					modelAndView.addObject("message","Given user id is already available. Please choose a different one!");
				}
			}
		}
		return modelAndView;
